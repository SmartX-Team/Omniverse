...
class CoordinateTransformer:
    ...
    def transform_uwb_to_omniverse_simple(self, uwb_x: float, uwb_y: float) -> Tuple[float, float, float]:
        """간단한 스케일링 변환 (기존 방식)"""
        precision = self.uwb_config.get('coordinate_precision', 2)
        
        # 기존 코드와 동일한 변환
        omni_x = round(uwb_y * -100, precision)  # Y -> X, 반전
        omni_z = round(uwb_x * 100, precision)   # X -> Z
    ...
    def transform_uwb_to_omniverse_advanced(self, uwb_x: float, uwb_y: float, tag_id: str = None) -> Tuple[float, float, float]:
        """고급 변환 - DB 매핑 정보 사용"""
        if not self._coordinate_mapping:
            print("Warning: No coordinate mapping available, using simple transform")
            return self.transform_uwb_to_omniverse_simple(uwb_x, uwb_y)
    ...
    async def transform_uwb_to_omniverse(self, uwb_x: float, uwb_y: float, tag_id: str = None) -> Tuple[float, float, float]:
        ...
        if self._coordinate_mapping:
            return self.transform_uwb_to_omniverse_advanced(uwb_x, uwb_y, tag_id)
        else:
            return self.transform_uwb_to_omniverse_simple(uwb_x, uwb_y)
    ...
    async def transform_and_save(self, tag_id: str, uwb_x: float, uwb_y: float, 
                               raw_timestamp: str) -> Tuple[float, float, float]:
        """좌표 변환 후 DB에 저장"""
        # Omniverse 좌표 변환
        omni_x, omni_y, omni_z = await self.transform_uwb_to_omniverse(uwb_x, uwb_y, tag_id)
        
        # GPS 좌표 변환 (선택사항)
        gps_coords = self.transform_uwb_to_gps(uwb_x, uwb_y)
